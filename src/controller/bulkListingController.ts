...
export const createBulkListing = async (req: AuthRequest, res: Response) => {
  try {
    const { platform, language, country, currency, products, priceAnalysisEnabled } = req.body;
    const files = req.files as Express.Multer.File[];
    const userId = req.userId;

    if (!platform || !language || !country || !currency || !products) {
      return res.status(400).json({ message: 'Missing required fields for bulk listing generation.' });
    }
    ...
    for (const product of parsedProducts) {
      ...
      const productImages = files.slice(fileIndex, fileIndex + product.images.length);
    ...
    res.status(500).json({ message: 'An unexpected error occurred while creating bulk listings.', error: error.message });
  }
};
